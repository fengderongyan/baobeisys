...
function GooFlow(bgDiv,property){
	...
	if(property.haveHead){
		...
		for(var x = 0; x < property.headBtns.length; ++x){
			tmp += "<a href='javascript:void(0)' class='GooFlow_head_btn' title='" + property.headBtnTitles[x] + "'><b class='ico_" + property.headBtns[x] + "'></b></a>";
		}
		tmp+="<span></span></div>";
		this.$head=$(tmp);
		...
		this.onOutPortClick = null; //重载导出功能
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_outport": if(This.onOutPortClick!=null) This.onOutPortClick();break;
			}
		});
	}
	var toolWidth = 0;
	if(property.haveTool){
		...
		if(property.toolBtns && property.toolBtns.length > 0){
			...
			for(var i=0; i < property.toolBtns.length; ++i){
				tmp+="<a href='javascript:void(0)' id='" + this.$id + "_btn_" + property.toolBtns[i] + "' class='GooFlow_tool_btn'><b class='ico_" + property.toolBtns[i] + "_tool'/></a>";//加入自定义按钮
			}
			this.$tool.append(tmp);
		}
		//加入分支和聚合结点,复合结点及区域划分框工具按钮
		if(property.haveGroup)
			this.$tool.append("<a href='javascript:void(0)' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><b class='ico_group'/></a>");
		toolWidth = 100;
		this.$nowType="cursor";
		//绑定各个按钮的点击事件
	...
	this.$bgDiv.append("<div class='GooFlow_work' oncontextmenu='window.event.returnValue=false;' style='width:"+(width)+"px;height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, {keyId : e.data.inthis.$nowType.toUpperCase() + "." + e.data.inthis.$max, name:"node_" + e.data.inthis.$max, left:X, top:Y, type:e.data.inthis.$nowType});
		e.data.inthis.$max++;
	  });
	  //划线时用的绑定
	  ...
	  //当操作某个单元右击
	  //格式function(id,type):id是单元的唯一标识ID,type是单元类型（"node"结点,"line"转换线）
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"contextmenu",{inthis:this},function(e){
			e.data.inthis.itemContextmenu(this.id,e,"line");
		});
		if(this.$editable)
	...
	setNodeRemarks:function(remark){
		if(this.$tool != null) {
			this.$tool.children("a").each(function(){
				$(this).append(remark[$(this).attr("id").split("btn_")[1]]);
	...
	addNode:function(id,json){
		...
		if(json.type != "start" && json.type != "end" && json.type != "exclusive" && json.type != "parallel" && json.type != "inclusive"){
			json.width=json.width? json.width:100;
			json.height=json.height? json.height:50;
			...
			json.width=40; json.height=40;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico_trans'><b class='ico_"+json.type+"'></b></td></tr></table><div style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","contextmenu",{inthis:this},function(e){
			e.data.inthis.focusItem(this.id,true);
			e.data.inthis.itemContextmenu(this.id,e,"node");
		});
		//绑定用鼠标移动事件
		this.$workArea.delegate(".ico,.ico_trans","mousedown",{inthis:this},function(e){
			if(!e)e=window.event;
			var This=e.data.inthis;
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart) This.addLine(This.$id+"_line_"+This.$max,{keyId: "ROUND." + This.$max, from:lineStart.id, to:this.id, name:""});
			This.$max++;
		});
		//绑定双击编辑事件
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			This.$textArea.val(oldTxt).css({display:"block",height:$(this).height(),width:100,
				...
				top:t.top+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop+42})
		...
		this.$workArea.delegate(".ico + td,.ico_trans + td","dblclick",{inthis:this},function(e){
			var oldTxt=this.innerHTML;
			var This=e.data.inthis;
	...
	//右击结点/转换线
	itemContextmenu:function(id,e,type){
		if(this.onItemContextmenu!=null&&!this.onItemContextmenu(id,type)) return false;
		/*
		$(this).removeClass("item_mark");

		if(type=="line") e.data.inthis.$lineOper.css({display:"none"}).data("tid",id);

		var ev=mousePosition(e);
		$("#item_menu").css({ left: ev.x+"px", top: ev.y+"px"});
		$("#item_menu").show();
		this.$focus=id;
	...
	focusItem:function(id,bool){
		...
		if(jq.prop("tagName")=="DIV"){
			...
			x=(parseInt(from[0]) + parseInt(to[0])) / 2 - 35;
			y=(parseInt(from[1]) + parseInt(to[1])) / 2 + 6;
	...
	setName:function(id,name,type){
		...
		if(type=="node"){//如果是结点
			...
			if(this.$nodeData[id].type=="start" || this.$nodeData[id].type=="end" || this.$nodeData[id].type=="exclusive" || this.$nodeData[id].type=="parallel" || this.$nodeData[id].type=="inclusive"){
...
///////////以下为有关画线的方法
	...
	getMValue:function(n1,n2,mType){
		if(mType=="lr"){
			return (Number(n1.left)+Number(n1.width)/2+Number(n2.left)+Number(n2.width)/2)/2;
		}
		else if(mType=="tb"){
			return (Number(n1.top)+Number(n1.height)/2+Number(n2.top)+Number(n2.height)/2)/2;
		}
	},
	//增加一条线
	addLine:function(id,json){
		...
		this.$lineData[id].keyId = json.keyId;
		...
		if(GooFlow.prototype.useSVG==""){
			...
			if(this.$lineData[id].type != "sl"){
				...
				this.$lineDom[id].childNodes[1].style.left = (Number(res.m2[0])+Number(res.m1[0]))/2-Number(Min)-Number(this.$lineDom[id].childNodes[1].offsetWidth)/2+4;
				Min=(res.start[1]>res.end[1]? res.end[1]:res.start[1]);
				if(Min>res.m2[1])	Min=res.m2[1];
				if(Min>res.m1[1])	Min=res.m1[1];
				this.$lineDom[id].childNodes[1].style.top = (Number(res.m2[1])+Number(res.m1[1]))/2-Number(Min)-Number(this.$lineDom[id].childNodes[1].offsetHeight)/2;
			}else
				this.$lineDom[id].childNodes[1].style.left =
				((Number(res.end[0])-Number(res.start[0]))*(Number(res.end[0])>Number(res.start[0])? 1:-1)-Number(this.$lineDom[id].childNodes[1].offsetWidth))/2+4;
		}
		...
		for(var i in this.$lineData){
		  ...
		  if(GooFlow.prototype.useSVG==""){
			...
			if(this.$lineData[i].type!="sl"){
				...
				if(Min>res.m2[0])	Min=res.m2[0];
				if(Min>res.m1[0])	Min=res.m1[0];
				this.$lineDom[i].childNodes[1].style.left = (Number(res.m2[0])+Number(res.m1[0]))/2-Number(Min)-Number(this.$lineDom[i].childNodes[1].offsetWidth)/2+4;
				Min=(res.start[1]>res.end[1]? res.end[1]:res.start[1]);
				if(Min>res.m2[1])	Min=res.m2[1];
				if(Min>res.m1[1])	Min=res.m1[1];
				this.$lineDom[i].childNodes[1].style.top = (Number(res.m2[1])+Number(res.m1[1]))/2-Number(Min)-Number(this.$lineDom[i].childNodes[1].offsetHeight)/2-4;
			}else
				this.$lineDom[i].childNodes[1].style.left=
				((Number(res.end[0])-Number(res.start[0]))*(Number(res.end[0])>Number(res.start[0])? 1:-1)-Number(this.$lineDom[i].childNodes[1].offsetWidth))/2+4;
		  }
	...
	//根据节点KeyId对流程进度进行标注
	markItemByKeyId:function(keyId, mark) {
		for(var id in this.$nodeData) {
			if(this.$nodeData[id].keyId == keyId) {
				if(mark) this.$nodeDom[id].addClass("item_mark_show");
				else this.$nodeDom[id].removeClass("item_mark_show");
			}
		}
	},
	////////////////////////以下为区域分组块操作
	...
	reinitSize:function(width, height){
		...
		h = h - headHeight - (this.$head != null ? 0 : 3);
