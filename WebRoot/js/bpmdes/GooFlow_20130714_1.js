...
function GooFlow(bgDiv, property){
	...
	this.$bgDiv.addClass("easyui-layout");
	this.$bgDiv.attr("data-options", "fit:true");
	...
	if(property.haveHead){
		tmp="<div data-options=\"region:'north'\" style=\"height:128px;\" toolbar=\"#head_tools\">";
		tmp += "<div id=\"head_tools\">";
		for(var x = 0; x < property.headBtns.length; ++x){
			tmp+="<a href=\"javascript:void(0)\" plain=\"true\" class=\"easyui-linkbutton\" iconCls=\"icon-" + property.headBtns[x] + "\">" + x + "</a>"
		}
		tmp += "</div>";
		tmp += "</div>";
		this.$head = $(tmp);
	...
	if(property.haveTool){
		this.$bgDiv.append("<div data-options=\"region:'east'\" " + (property.haveHead? "" : " style='margin-top:3px'") + " id=\"tool_btns\"></div>");
		this.$tool=this.$bgDiv.find("#tool_btns");
		//未加代码：加入绘图工具按钮
		this.$tool.append("<a href='javascript:void(0)' class=\"easyui-linkbutton\" id='" + this.$id + "_btn_cursor' plain=\"true\" iconCls=\"ico_cursor\">选择</a><br/>");
		this.$tool.append("<a href='javascript:void(0)' class=\"easyui-linkbutton\" id='" + this.$id + "_btn_direct' plain=\"true\" iconCls=\"ico_direct\">连线</a><br/>");
		tmp = "";
		if (property.toolBtns && property.toolBtns.length>0) {
			for(var i = 0; i < property.toolBtns.length; ++i){
				tmp += "<a href='javascript:void(0)' id='" + this.$id + "_btn_" + property.toolBtns[i] + "' plain=\"true\" class=\"easyui-linkbutton\" iconCls='ico_" + property.toolBtns[i] + "'></a><br/>";//加入自定义按钮
			}
			this.$tool.append(tmp);
		}
		prompt("1", document.getElementById("demo").outerHTML);
	...
	//this.$bgDiv.append("<div data-options=\"region:'center'\" style='"+(property.haveHead?"":"margin-top:3px")+"'></div>");
	this.$workArea = $("<div data-options=\"region:'center'\" id=\"work_area\">123123</div>")
		.attr({"unselectable" : "on", "onselectstart" : 'return false', "onselect" : 'document.selection.empty()'});
	this.$bgDiv.append(this.$workArea);
