...
(function($){
    ...
    $.fn.wBox = function(options){
        ...
        var  wBoxHtml = '<div id="wBox"><div class="wBox_popup"><table><tbody><tr><td class="wBox_tl"/><td class="wBox_b"/><td class="wBox_tr"/></tr><tr><td class="wBox_b"><div>&nbsp;</div></td><td class="wBox_body">' + (_this.YQ.noTitle ? '' : '<table class="wBox_title"><tr><td class="wBox_dragTitle"><div class="wBox_itemTitle">' + _this.YQ.title + '</div></td><td width="20px" title="关闭"><div class="wBox_close"></div></td></tr></table> ') +
        '<div class="wBox_content" id="wBoxContent"></div></td><td class="wBox_b"><div>&nbsp;</div></td></tr><tr><td class="wBox_bl"/><td class="wBox_b"/><td class="wBox_br"/></tr></tbody></table></div></div>', B = null, C = null, $win = $(window),$t=$(this);//B背景，C内容jquery div   
        ...
        function handleClick(){
			...
			if (_this.YQ.requestType && $.inArray(_this.YQ.requestType, ['iframe', 'ajax','img'])!=-1) {
				con.html("<div id='wBox_loading'><img src='"+_this.YQ.wBoxURL+"loading.gif' /></div>");				
				if (_this.YQ.requestType === "img") {
					var img = $("<img />");
					img.attr("src",_this.YQ.target);
					img.load(function(){
						img.replaceAll(con.find("#wBox_loading"));
						setPosition();
					});
				}
				else 
				...
				if (_this.YQ.target) {
					...
					$(_this.YQ.target).empty();
					con.replaceWith(clone_html);
				}
				else 
					if (_this.YQ.html) {
						...
						con.replaceWith($t.clone(true).show());
					}         
            afterHandleClick();
        }
        /*
        ...
        function setPosition(){
            ...
            var $h = $("html").height(), $wh = $win.height();
            $h = Math.max($h, $wh);
        ...
        this.close=function (){
        	if (_this.YQ.target) {
				$(_this.YQ.target).html(C.find(_this.YQ.target).html());
			}
            if (C) {
                B.remove();
